...
export default function BookingForm({ flight, onBooked }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSuccess(null);
    try {
      if (!user?.token) throw new Error("Please login to book a flight");
    ...
    <form className="mt-4 p-4 border rounded bg-gray-50" onSubmit={handleSubmit}>
      ...
      {success && (
        <div className="text-green-600 border border-green-200 bg-green-50 p-2 rounded mt-2">
          ...
          <a href={success.pdf_url ? apiUrl(success.pdf_url) : "#"} target="_blank" rel="noopener noreferrer" className="underline text-blue-600">Download Ticket (PDF)</a>
        </div>
      )}
    </form>
  );
